...
### In Progress
- A new `mobile/` React Native (Expo) app, scaffolded but not yet verified end-to-end. It wraps the existing trader PWA in a WebView and integrates `llama.rn` (the React Native binding for llama.cpp) to run a small on-device LLM for one narrow job: narrating an already-computed ITC verdict in natural language (Hindi/English) as a fully offline alternative to the existing Gemini-based explanation call. Does not touch OCR (still Gemini vision, server-side) and does not touch any compliance/verdict logic (still 100% deterministic backend Python) — a narration path for output the backend already computes, nothing more. Two things are unverified: llama.cpp/llama.rn's Hexagon NPU backend is experimental and needs Snapdragon 8 Gen 2+, while the actual hackathon phones are loaner devices issued at check-in with unknown chipset, so NPU execution isn't confirmed and may not be available at all — the CPU/Adreno-GPU fallback llama.rn uses automatically is what actually satisfies "local model at the core" regardless of chipset; and a full build/install needs an Expo/EAS account login (`eas login`, interactive OAuth) plus a physical device for final verification, neither of which exists in the current dev environment. This is `NPU-1` from `IQOO_DEVICE_CAPABILITY_SPEC.md`, built as on-device LLM verdict narration rather than the originally-catalogued on-device semantic HSN match.
